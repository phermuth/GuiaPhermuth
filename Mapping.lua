...
local function MapPoint(zone, x, y, desc)
	GuiaPhermuth:Debug( string.format("Mapping %q - %s (%.2f, %.2f)", desc, zone, x, y))
	local zi, zc = zone and zonei[zone], zone and zonec[zone]
	if not zi then
		if zone then GuiaPhermuth:Print(L["Cannot find zone %q, using current zone."], zone)
		else GuiaPhermuth:Print(L["No zone provided, using current zone."]) end
...
function GuiaPhermuth:MapPfQuestNPC(qid, action)
...
function GuiaPhermuth:MapLightHeadedNPC(qid, action)
	if not self.db.char.mapquestgivers then return end
...
function GuiaPhermuth:ParseAndMapCoords(qid, action, note, desc, zone)
	if TomTom then
		local TomTom = TomTom
