...
local function ShowTooltip()
	local f = this
	if GuiaPhermuth.db.char.completion[f.guide] ~= 1 then return end
...
local function OnClick()
	...
	if IsShiftKeyDown() then
		GuiaPhermuth.db.char.completion[f.guide] = nil
		GuiaPhermuth.db.char.turnins[f.guide] = {}
		GuiaPhermuth:UpdateGuideListPanel()
		GameTooltip:Hide()
	else
		local text = f.guide
		if not text then f:SetChecked(false)
		else
			GuiaPhermuth:LoadGuide(text)
			GuiaPhermuth:UpdateStatusFrame()
			GuiaPhermuth:UpdateGuideListPanel()
		end
	end
end

local frame = CreateFrame("Frame", "GuiaPhermuthGuideList", GuiaPhermuth.statusframe)
GuiaPhermuth.guidelistframe = frame
...
-- Rellene el marco con los botones de verificación de los guías.
...
frame:SetScript("OnShow", function()
	local quad, vhalf, hhalf = GuiaPhermuth.GetQuadrant(GuiaPhermuth.statusframe)
	local anchpoint = (vhalf == "TOP" and "BOTTOM" or "TOP")..hhalf
	this:ClearAllPoints()
	this:SetPoint(quad, GuiaPhermuth.statusframe, anchpoint)
	GuiaPhermuth:UpdateGuideListPanel()
	this:SetAlpha(0)
...
frame:SetScript("OnMouseWheel", function()
	...
	if (offset + NUMROWS*2) > table.getn(GuiaPhermuth.guidelist) then offset = offset - NUMROWS end
...
table.insert(UISpecialFrames, "GuiaPhermuthGuideList")

function GuiaPhermuth:UpdateGuideListPanel()
