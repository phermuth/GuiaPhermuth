...
local GuiaPhermuth = GuiaPhermuth
local ww = WidgetWarlock

local f = CreateFrame("Button", nil, UIParent)
GuiaPhermuth.statusframe = f
...
function GuiaPhermuth:HideStatusFrameChildren()
	if GuiaPhermuth.objectiveframe:IsVisible() then HideUIPanel(GuiaPhermuth.objectiveframe) end
	if GuiaPhermuth.optionsframe:IsVisible() then HideUIPanel(GuiaPhermuth.optionsframe) end
	if GuiaPhermuth.guidelistframe:IsVisible() then HideUIPanel(GuiaPhermuth.guidelistframe) end	
end

function GuiaPhermuth:PositionStatusFrame()
...
function GuiaPhermuth:SetStatusText(i)
	self.current = i
...
function GuiaPhermuth:UpdateStatusFrame()
...
function GuiaPhermuth:PLAYER_REGEN_ENABLED()
	if tex then
...
f:SetScript("OnClick", function()
	...
	if GuiaPhermuth.db.char.currentguide == "No Guide" then
		GuiaPhermuth.guidelistframe:Show()
	else
		if btn == "RightButton" then
			if GuiaPhermuth.objectiveframe:IsVisible() then
				HideUIPanel(GuiaPhermuth.objectiveframe)
			else
				local quad, vhalf, hhalf = GuiaPhermuth.GetQuadrant(self)
				local anchpoint = (vhalf == "TOP" and "BOTTOM" or "TOP")..hhalf
				GuiaPhermuth.objectiveframe:ClearAllPoints()
				GuiaPhermuth.objectiveframe:SetPoint(quad, self, anchpoint)
				ShowUIPanel(GuiaPhermuth.objectiveframe)
			end
		else
			if QuestLogFrame:IsVisible() or (EQL3_QuestLogFrame and EQL3_QuestLogFrame:IsVisible()) then
				...
				local i = GuiaPhermuth:GetQuestLogIndexByName()
...
check:SetScript("OnClick", function(self, btn) GuiaPhermuth:SetTurnedIn() end)

item:SetScript("OnClick", function()
	if GuiaPhermuth:GetObjectiveInfo() == "USE" then GuiaPhermuth:SetTurnedIn() end
	if item.uitem then
		local bag, slot = GuiaPhermuth:FindBagSlot(item.uitem)
		if bag and slot then UseContainerItem(bag, slot) else GuiaPhermuth:Print("Item not found") end
	end
end)

local function ShowTooltip()
	local self = this
	local tip = GuiaPhermuth:GetObjectiveTag("N")
	if not tip or tip == "" then return end
	tip = tostring(tip)
	local quad, vhalf, hhalf = GuiaPhermuth.GetQuadrant(self)
	--local anchpoint = (vhalf == "TOP" and "BOTTOM" or "TOP")..hhalf
	local anchpoint = "ANCHOR_TOP"..hhalf
	GuiaPhermuth:Debug( "Setting tooltip anchor", anchpoint)
...
f:SetScript("OnDragStart", function()
	local frame = this
	GuiaPhermuth:HideStatusFrameChildren()
	GameTooltip:Hide()
	frame:StartMoving()
end)
f:SetScript("OnDragStop", function()
	...
	local _
	GuiaPhermuth.db.profile.statusframepoint, _, _, GuiaPhermuth.db.profile.statusframex, GuiaPhermuth.db.profile.statusframey = frame:GetPoint()
	frame:ClearAllPoints()
	frame:SetPoint(GuiaPhermuth.db.profile.statusframepoint, GuiaPhermuth.db.profile.statusframex, GuiaPhermuth.db.profile.statusframey)
	ShowTooltip(frame)
end)
...
item:SetScript("OnDragStop", function()
	...
	frame:StopMovingOrSizing()
	local _
	GuiaPhermuth.db.profile.itemframepoint, _, _, GuiaPhermuth.db.profile.itemframex, GuiaPhermuth.db.profile.itemframey = frame:GetPoint()
end)
