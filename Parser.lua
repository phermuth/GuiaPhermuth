...
function GuiaPhermuth:GetObjectiveTag(tag, i)
...
local function DumpQuestDebug(accepts, turnins, completes)
	for quest in pairs(accepts) do if not turnins[quest] then GuiaPhermuth:Debug( string.format("Quest has no 'turnin' objective: %s", quest)) end end
	for quest in pairs(turnins) do if not accepts[quest] then GuiaPhermuth:Debug( string.format("Quest has no 'accept' objective: %s", quest)) end end
	for quest in pairs(completes) do if not accepts[quest] and not turnins[quest] then GuiaPhermuth:Debug( string.format("Quest has no 'accept' and 'turnin' objectives: %s", quest)) end end
end
...
local function DebugQuestObjective(text, action, quest, accepts, turnins, completes)
	...
	if (action == "A" and accepts[quest] or action == "T" and turnins[quest] or action == "C" and completes[quest]) and not string.find(text, "|NODEBUG|") then
		GuiaPhermuth:Debug( string.format("%s %s -- Duplicate objective", action, quest))
		haserrors = true
	end
	...
	if action == "A" or action == "C" or action == "T" then
		...
		for _,word in pairs(titlematches) do
			if string.find(quest,"[^:]%s"..word.."%s") or string.find(quest,"[^:]%s"..word.."$") or string.find(quest,"[^:]%s"..word.."@") then
				GuiaPhermuth:Debug( string.format("%s %s -- Contains bad title case", action, quest))
				haserrors = true
			end
		end
	end
	...
	if string.find(text,"[���]") then
		GuiaPhermuth:Debug( string.format("%s %s -- Contains bad char", action, quest))
		haserrors = true
	end

	local _, _, comment = string.find(text, "(|[NLUC]V?|[^|]+)$") or string.find(text, "(|[NLUC]V?|[^|]+) |[NLUC]V?|")
	if comment then
		GuiaPhermuth:Debug( "Unclosed comment: ".. comment)
		haserrors = true
	end
...
local function StepParse(guide)
	...
	local guidet = GuiaPhermuth.split("\r\n", guide)
	...
	if haserrors and GuiaPhermuth:IsDebugging() then GuiaPhermuth:Print("This guide contains errors") end

	return actions, quests, tags
end


function GuiaPhermuth:LoadGuide(name, complete)
...
function GuiaPhermuth:DebugGuideSequence(dumpquests)
	local accepts, turnins, completes = {}, {}, {}
	local function DebugParse(guide)
		...
		for _,text in pairs(guidet) do
			if text ~= "" then
				local _, _, action, quest, tag = string.find(text,"^(%a) ([^|]*)(.*)")
				if not actiontypes[action] then GuiaPhermuth:Debug( "Unknown action: "..text) end
				quest = GuiaPhermuth.trim(quest)
				if not (action == "A" or action =="C" or action =="T") then
					quest = quest.."@"..uniqueid.."@"
					uniqueid = uniqueid + 1
